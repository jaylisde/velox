...
TEST_F(ParquetTableScanTest, deltaBinaryPackedFilterScalarTail) {
  // Regression: when readWithVisitorDenseBatched calls processRun with
  // numInput < kWidth (SIMD batch size), the SIMD loop never runs and the
  // scalar tail handles all rows. The tail had been calling visitor.process()
  // which advances reader.numValues_ directly; the surrounding caller then
  // calls setNumValues(local) at the end with a local counter that the tail
  // never updated, silently dropping every row the tail produced. Reproduces
  // by chaining filters so that one column's reader receives a single-value
  // batch (total < kWidth) whose row would otherwise pass.
  WriterOptions options;
  options.enableDictionary = false;
  options.encoding =
      facebook::velox::parquet::arrow::Encoding::kDeltaBinaryPacked;
  ...
  // a BETWEEN 10000 AND 70000 keeps exactly 1 row in some 10K-row scan
  // window (row 70000), forcing b's reader to processRun a 1-row batch
  // that lands entirely in the scalar tail.
  assertSelectWithFilter(
      {makeSplit(file->getPath())},
      {"a", "b"},
