...
title: "From copyBits to SIMD: Parquet DELTA Decoding in Velox"
date: 2026-06-17
...
Velox's Parquet DELTA decoders were ~8× slower than the PLAIN/dictionary
path, on a TPC-H Q12 scan over a DELTA-encoded `lineitem`, decoding cost
~5.0 s of CPU versus ~0.6 s for the same data stored as PLAIN/dictionary.
Chasing that gap led through two fixes: replacing a general-purpose bit-copy
with a single unaligned load,
then a batched decode path with SIMD bit-unpack kernels. Together they cut Q12
scan CPU from **5.0 s to 1.1 s** and end-to-end wall time **1.71 s to 0.59 s
(2.9×)**.
...
CPU on DELTA decoding itself. That was the thing to fix.
...
`readLong` runs once per value, so collapsing a byte-loop to one load mattered:
on its own it took Q12 scan CPU from **5.0 s to 2.8 s** and wall time
**1.71 s to 1.02 s**.

## Still 4.5× off: the unpack was still scalar

The single-load fix helped, but DELTA scan CPU was still ~4.5× the
...
per row, paying per-row dispatch even when no filter was active. Each needed
its own fix.
...
### Calling the visitor once per chunk
...
one sequential, cache-friendly pass followed by array indexing beats seeking
per row.

### And the string decoder
...
The first fix is a **1.7× wall** improvement; the batched SIMD path adds another
**−42% wall / −60% scan CPU** on top. The DELTA-vs-PLAIN scan-CPU gap shrank
